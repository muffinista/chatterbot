...
application with Twitter.  Twitter all API communication to be via an
...
4. Take the consumer key/consumer secret values, and either run your
   bot, and enter them in when prompted, or place them in global.yml
   or in a YAML file specific to your bot.  It should look like this:
...
Once this is done, you will need to setup authorization with
Twitter. At the first run, you'll get a message asking you to go
...
5. In a database.  You can store your configuration in a DB, and then
   ...
   files, or on the command-line by using the +db+
   configuration option.  Any calls to the database are handled by the
   Sequel gem, and MySQL and Sqlite should work.

=== Database logging

Chatterbot can log tweet activity to the database if desired.  This
can be handy for tracking what's going on with your bot.  See +Chatterbot+::+Logging+ for details on this.
...
* some sort of web-based frontend for tracking bot activity
